...
export function ParallaxBackground({ targetRef }: { targetRef?: React.RefObject<HTMLElement> }) {
  ...
  return (
    <motion.div aria-hidden className="pointer-events-none fixed inset-0 -z-10 overflow-hidden" style={{ background }}>
...
function SkyLayer({ progress }: { progress: any }) {
  ...
  return (
    <motion.div className="absolute inset-0" style={{ y, filter: "blur(1px)" }}>
      <motion.div
...
function SunLayer({ sunTop, sunOpacity, sunScale }: { sunTop: any; sunOpacity: any; sunScale: any }) {
  ...
  return (
    <motion.div className="absolute left-1/2 -translate-x-1/2" style={{ top: useTransform(sunTop, (v: number) => `${v}%`), opacity: sunOpacity, scale: sunScale }}>
      <motion.div
...
function RoofPanelLayer({ progress, sunTop, sunOpacity }: { progress: any; sunTop: any; sunOpacity: any }) {
  const y = useTransform(progress, [0, 1], [0, -250]);
  ...
  return (
    <motion.svg viewBox="0 0 1440 220" preserveAspectRatio="none" className="absolute bottom-0 left-0 w-full" style={{ y, opacity: horizonOpacity }}>
      <defs>
